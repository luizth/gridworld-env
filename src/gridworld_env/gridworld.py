...
class GridWorld:
    def __init__(
        ...
        initial_and_goal_states: Optional[Tuple[State, State]] = None,
    ...
    def save_room(self, base_path: str) -> None:
    ...
    def get_one_hot_state(self, state: State) -> OneHotVector:
        one_hot_state = np.zeros(self.num_states, dtype=float)
        idx = self.state_coordinates_to_idx[state]
        one_hot_state[idx] = 1.0
    ...
    def get_one_hot_state_action(self, state: State, action: Actions) -> OneHotVector:
        one_hot_state_action = np.zeros(self.num_states * self.num_actions, dtype=float)
        state_idx = self.state_coordinates_to_idx[state]
        action_idx = int(action)
        idx = state_idx * self.num_actions + action_idx
        one_hot_state_action[idx] = 1.0
    ...
    def plot_policy(
        self,
        policy_probs: NDArray[float],
        subgoal_state: Optional[State] = None,
        file_name=None,
        only_max: bool = False,
    ) -> None:
        ...
        # Arrow properties
        arrow_props = dict(
            head_width=0.3, head_length=0.3, fc="black", ec="black", alpha=1.0
        )
        ...
        for state_idx, coords in self.state_idx_to_coordinates.items():
            x, y = coords
            if x == subgoal_state[0] and y == subgoal_state[1]:
                continue
            ...
            if only_max:
                ...
                # plt.text(
                #     x, y + 0.4, f"{prob:.2f}", ha="center", va="bottom", fontsize=8
                # )
            else:
    ...
    def __get_initial_and_goal_states(
        self, initial_and_goal_states: Optional[Tuple[State, State]] = None
    ) -> Tuple[State, State]:
